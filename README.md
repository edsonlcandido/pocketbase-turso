...
*   Uma conta no [Turso](https://turso.tech/) e **dois bancos de dados** criados.
*   URL e Token dos seus bancos de dados Turso.
...
2.  **Crie dois bancos no Turso:**
    - um banco principal para o `data.db`
    - um banco auxiliar para o `auxiliary.db`
...
3.  **Configure suas credenciais:**
    ...
    | `URL_LIBSQL_TURSO_AUX` | ✅ Sim | URL do banco auxiliar (`auxiliary.db`). Deve apontar para um **segundo database separado** no Turso para evitar colisão de tabelas com `data.db`. |
...
*   `/pb_data`: Pasta criada automaticamente pelo PocketBase.
    ...
    *   `auxiliary.db`: Conectado ao banco auxiliar no Turso via `URL_LIBSQL_TURSO_AUX`. **É obrigatório usar um database separado no Turso** para evitar colisão de tabelas com `data.db`.
...
2.  **Isolamento dos bancos:** `data.db` e `auxiliary.db` devem sempre apontar para databases distintos no Turso.
3.  **Segurança:** Evite subir seu `main.go` para repositórios públicos com o Token do Turso exposto. Recomenda-se usar variáveis de ambiente em produção.
4.  **CGO:** Utilizamos `CGO_ENABLED=0` durante o build para garantir que o executável seja estático e funcione em qualquer servidor sem depender de bibliotecas do sistema.